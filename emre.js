...
*/
client.conf = {
  "token": "NjkxNzAzNDcyNzA5Njk3NTU2.XphGpA.gs7hvAI0rHEDtcWDe3waEzwCOCU",
  "pref": "ck!",
...
client.on("message", async msg => {
 ...
 const i = await db.fetch(`${msg.guild.id}.kufur`)
    if (i) {
        const kufur = ["oç", "amk", "ananı sikiyim", "ananıskm", "piç","sikim", "sikiyim", "orospu çocuğu", "piç kurusu", "orospu",  "sik", "yarrak", "amcık",  "yarram", "sikimi ye",  "aq", "amq","göt lalesi"];
...
client.on('message', message => {
  ...
  if (message.content.toLowerCase() === '<@&693909163025367100>' || message.content.toLowerCase() === '<@&693909163025367100>' || message.content.toLowerCase() === '<@&693909163025367100>') {
  message.delete()
  message.channel.send(`${message.author}, bu rolü etiketleyemezsin.`).then(message => message.delete(6000)) }
})

setInterval(() => {
  client.channels.get("693912473836716042").send('**Otomatik Duyuru:** Sunucumuzda Küfür,Dil,Din,Irk Ayrımı Kesinlikle Yasaktır.')
}, 3900000)
